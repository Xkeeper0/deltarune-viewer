if (self.state == 3)
{
    if (global.monsterhp[self.myself] <= (global.monstermaxhp[self.myself] / 3))
    {
        ...
        if (global.monstercomment[self.myself] == " "@24)
            global.monstercomment[self.myself] = scr_84_get_lang_string("obj_heartenemy_slash_Draw_0_gml_6_0"@9113)
    }
    ...
    else
    {
        if (global.monster[self.myself] == 0)
        {
            if (global.flag[(51 + self.myself)] == 5)
                global.flag[524] = (global.flag[524] + 1)
            global.flag[521] = (global.flag[521] + 1)
            scr_defeatrun()
        }
        ...
        draw_sprite_ext(spr_heartenemy_hurt, 0, (self.x + self.shakex), self.y, 2, 2, 0, self.image_blend, 1)
    }
}
if (self.state == 0)
{
    self.siner = (self.siner + 1)
    self.thissprite = spr_heartenemy_idle
    if (global.mercymod[self.myself] >= global.mercymax[self.myself])
        self.thissprite = spr_heartenemy_spared
