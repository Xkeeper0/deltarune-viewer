...
if (self.chaincon == 2)
{
    ...
    self.chaintimer = (self.chaintimer + 1)
    if (self.chaintimer >= self.chainrate)
    {
        ...
        self.chain_max = (self.chain_max + 1)
        self.chain_number = (self.chain_number + 1)
        self.chaintimer = 0
    }
    ...
    if (self.type == 88)
    {
        ...
        if (self.btimer >= 2)
            _temp_local_var_1 = (self.path_position <= 0.8)
        else
            _temp_local_var_1 = 0
...
if (self.chaincon == 2.1)
{
    ...
    obj_growtangle.x = (obj_growtangle.x + self.xshake)
    obj_growtangle.y = (obj_growtangle.y + self.yshake)
...
if (self.chaincon == 6)
{
    path_start(self.mypath2, (self.maxspeed / self.factor), 0, 0)
    ...
    if (self.type == 0)
    {
        ...
        if (self.btimer == 10)
        {
            ...
            while(true)
            {
                if (self.i < 1)
                {
                    self.regbul = instance_create(((__view_get(e__VW.XView, 0) + 320) + (self.i * 120)), (__view_get(e__VW.YView, 0) - 20), obj_regularbullet)
        ...
        if (self.btimer >= 20)
        {
            ...
            while(true)
            {
                if (self.i < 2)
                {
                    self.regbul = instance_create(((__view_get(e__VW.XView, 0) + 260) + (self.i * 120)), (__view_get(e__VW.YView, 0) - 20), obj_regularbullet)
                    self.regbul.vspeed = 2
                    self.regbul.gravity = 0.2
                    self.regbul.sprite_index = spr_spadebullet
                    self.regbul.image_angle = 270
