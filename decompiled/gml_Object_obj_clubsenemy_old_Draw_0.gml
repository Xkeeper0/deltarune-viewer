if (self.state == 3)
{
    if (global.monsterhp[self.myself] <= (global.monstermaxhp[self.myself] / 2))
    {
        ...
        if (global.monstercomment[self.myself] == " "@24)
            global.monstercomment[self.myself] = scr_84_get_lang_string("obj_clubsenemy_old_slash_Draw_0_gml_6_0"@9079)
    }
    ...
    else
    {
        ...
        {
            if (global.mercymod[self.myself] > 0)
                _temp_local_var_1 = 1
            else
                _temp_local_var_1 = (self.battlecancel == 1)
