self.idletimer = 0
self.idlefacer = 0
self.talkfacer = 0
self.talkbuffer = 0
self.menu = 0
self.submenu = 0
global.typer = 6
draw_set_color(0xFFFFFF)
scr_84_set_draw_font("mainbig"@1561)
self.talktimer = 0
self.cur_jewel = 0
self.shopcharx = 0
self.siner = 0
self.i = 0
while(true)
{
    if (self.i < 20)
    {
        self.menuc[self.i] = 0
        self.submenuc[self.i] = 0
        self.i = (self.i + 1)
        continue
    }
    break
}
self.onebuffer = 0
self.twobuffer = 0
self.upbuffer = 0
self.downbuffer = 0
self.hold_up = 0
self.hold_down = 0
self._up_pressed = 0
self._down_pressed = 0
self.murder = 0
self.moff = 415
global.currentsong[0] = snd_init("hip_shop.ogg"@9397)
global.currentsong[1] = mus_loop_ext(global.currentsong[0], 1, 1)
self.menu = 0
self.menuc[0] = 0
self.menuc[1] = 0
self.menuc[2] = 0
self.menuc[3] = 0
self.menuc[4] = 0
self.item0pic = 908
self.item1pic = 908
self.item2pic = 908
self.item3pic = 908
self.itemtotal = 4
self.item[0] = 15
self.item[1] = 6
self.item[2] = 10
self.item[3] = 1
self.item[4] = 0
self.item[5] = 0
self.item[6] = 0
self.itemtype[0] = "item"@103
self.itemtype[1] = "weapon"@166
self.itemtype[2] = "weapon"@166
self.itemtype[3] = "armor"@167
self.itemtype[4] = "item"@103
self.itemtype[5] = "item"@103
self.itemtype[6] = "item"@103
self.shopdesc[0] = scr_84_get_lang_string("obj_shop2_slash_Create_0_gml_73_0"@9398)
self.shopdesc[1] = scr_84_get_lang_string("obj_shop2_slash_Create_0_gml_74_0"@9399)
self.shopdesc[2] = scr_84_get_lang_string("obj_shop2_slash_Create_0_gml_75_0"@9400)
self.shopdesc[3] = scr_84_get_lang_string("obj_shop2_slash_Create_0_gml_76_0"@9401)
self.i = 0
while(true)
{
    if (self.i < self.itemtotal)
    {
        self.itematk[self.i] = 0
        self.itemdef[self.i] = 0
        self.itemmagic[self.i] = 0
        self.canequip[self.i][1] = 0
        self.canequip[self.i][2] = 0
        self.canequip[self.i][3] = 0
        if (self.itemtype[self.i] == "item"@103)
        {
            scr_iteminfo(self.item[self.i])
            self.shopitemname[self.i] = self.itemnameb
            self.buyvalue[self.i] = self.value
        }
        if (self.itemtype[self.i] == "armor"@167)
        {
            scr_armorinfo(self.item[self.i])
            self.shopitemname[self.i] = self.armornametemp
            self.buyvalue[self.i] = self.value
            self.itemdef[self.i] = self.armordftemp
            self.canequip[self.i][1] = self.armorchar1temp
            self.canequip[self.i][2] = self.armorchar2temp
            self.canequip[self.i][3] = self.armorchar3temp
        }
        if (self.itemtype[self.i] == "weapon"@166)
        {
            scr_weaponinfo(self.item[self.i])
            self.itematk[self.i] = self.weaponattemp
            self.itemmagic[self.i] = self.weaponmagtemp
            self.shopitemname[self.i] = self.weaponnametemp
            self.canequip[self.i][1] = self.weaponchar1temp
            self.canequip[self.i][2] = self.weaponchar2temp
            self.canequip[self.i][3] = self.weaponchar3temp
            self.buyvalue[self.i] = self.value
        }
        self.i = (self.i + 1)
        continue
    }
    break
}
self.sell = 0
self.bought = 0
self.mainmessage = 0
self.minimenuy = 220
global.typer = 23
scr_84_set_draw_font("mainbig"@1561)
self.sidemessage = 0
self.selling = 0
global.msc = 0
self.glow = 0
self.shx = 130
self.soldo = 0
global.faceemotion = 0
self.an = 0
global.fe = 0
self.siner = 0
self.friendly = 0
if (global.plot >= 175)
    self.friendly = 1
self.worm_max = 4
self.i = 0
while(true)
{
    if (self.i < self.worm_max)
    {
        self.worm_siner[self.i] = random(999)
        self.worm_image[self.i] = floor(random(4))
        self.worm_y[self.i] = (-4 + random(8))
        self.i = (self.i + 1)
        continue
    }
    break
}
