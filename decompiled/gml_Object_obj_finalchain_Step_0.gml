if (self.chaincon == 0)
{
    ...
    if (self.sons < 40)
    {
        ...
        self.son[self.sons].image_blend = 0x000000FF
    }
...
{
    self.xx = (__view_get(0, 0) + 320)
    self.yy = (__view_get(1, 0) + 160)
    self.yy_2 = __view_get(1, 0)
    self.chaincon = 2.3
    if (self.type >= 1)
    {
    ...
    if (self.type <= 1)
    {
        ...
        {
            for (self.j = 0; self.j < 5; self.j += 1)
            {
...
if (self.chaincon == 2.3)
{
    ...
    if (self.spikemake == 1)
    {
        self.box.image_xscale += 0.005
        self.box.image_yscale += 0.005
        self.spike.image_xscale += 0.005
        self.spike.image_yscale += 0.005
        if (self.type == 1)
        {
            self.box.image_xscale += 0.01
            self.box.image_yscale += 0.01
            self.spike.image_xscale += 0.01
            self.spike.image_yscale += 0.01
        }
    }
    self.gotimer += 1
    if (self.gotimer >= 30)
...
if (self.chaincon == 3)
{
    if ((self.movecon == 0) && (self.ended == 0))
    {
    ...
    if ((self.movecon == 0) && (self.ended == 0))
    {
        ...
        while ((self.pointdist < self.mind) || (self.pointdist > self.maxd))
        {
    ...
    if ((self.movecon == 1) && (self.ended == 0))
    {
        self.movetimer += 1
    ...
    if ((self.movecon == 2) && (self.ended == 0))
    {
        self.maxmove = 20
        if (self.type == 1)
            self.maxmove = 22
...
if (self.bulletpoint == 1)
{
    with (obj_regularbullet)
    {
        if (self.sprite_index == spr_spadebullet)
        {
            ...
            if ((self.image_xscale < 0.68) && (obj_finalchain.ended == 0))
            {
                self.image_xscale += 0.0005
                self.image_yscale += 0.0005
            }
            else
            {
                self.active = 0
